...
WORKDIR /src
COPY ["NotificadorMain.csproj", "NotificadorMain/"]
RUN dotnet restore "NotificadorMain/NotificadorMain.csproj"

WORKDIR "/src/NotificadorMain"
COPY . .
RUN dotnet build "NotificadorMain.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "NotificadorMain.csproj" \
...
Execute unit tests
```
cd "NotificadorMain/test/NotificadorMain.Tests"
dotnet test
```

Deploy function to AWS Lambda
```
cd "NotificadorMain/src/NotificadorMain"
